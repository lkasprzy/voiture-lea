<?php
/**
 * voiture functions and definitions
 *
...
if ( ! function_exists( 'voiture_setup' ) ) :
	/**
	 ...
	 */
	function voiture_setup() {
		/*
		 * Make theme available for translation.
		 * Translations can be filed in the /languages/ directory.
		 * If you're building a theme based on voiture, use a find and replace
		 * to change 'voiture' to the name of your theme in all the template files.
		 */
		load_theme_textdomain( 'voiture', get_template_directory() . '/languages' );
		...
		register_nav_menus(
			array(
				'menu-1' => esc_html__( 'Primary', 'voiture' ),
			)
		);
		...
		add_theme_support(
			'custom-background',
			apply_filters(
				'voiture_custom_background_args',
				array(
...
add_action( 'after_setup_theme', 'voiture_setup' );
 ...
 */
function voiture_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'voiture_content_width', 640 );
}
add_action( 'after_setup_theme', 'voiture_content_width', 0 );
 ...
 */
function voiture_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'voiture' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'voiture' ),
...
add_action( 'widgets_init', 'voiture_widgets_init' );

/**
 * Enqueue scripts and styles.
 */
function voiture_scripts() {
	wp_enqueue_style( 'voiture-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'voiture-style', 'rtl', 'replace' );

	wp_enqueue_script( 'voiture-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );

	if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
		wp_enqueue_script( 'comment-reply' );
	}
}
add_action( 'wp_enqueue_scripts', 'voiture_scripts' );
